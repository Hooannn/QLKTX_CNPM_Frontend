...
import { BookingTime, IResponseData, Region, Role, Room } from "../../types";
import {
    Accordion,
    AccordionItem,
    Button,
...
function FormStep({
    ...
    const getBookingTimeQuery = useQuery({
        ...
        refetchOnWindowFocus: false,
        queryFn: () => {
            return axios.get<IResponseData<BookingTime[]>>(`/api/v1/booking/booking-time/available`);
        },
        onSuccess(data) {
    ...
    return (
        <Card shadow="sm" radius="sm" className="mx-auto w-full max-w-[500px]">
            ...
            {
                getBookingTimeQuery.isLoading ? <CardBody className="flex items-center justify-center py-20">
                    <Spinner size='lg' />
                </CardBody> : <>
                    {
                        bookingTimes.length > 0 ? <>
                            <CardBody className="gap-4">
                                ...
                                <div>
                                    ...
                                    }} defaultSelectedKeys={[selectedTime ?? ""]} value={selectedTime} size="sm" className="pt-2">
                                        {bookingTimes.map((bookingTime) => (
                                            <SelectItem key={bookingTime.id} value={bookingTime.description}>
                            ...
                            <CardFooter>
                                <div className="flex justify-center w-full">
                                    <Button size="lg" color="primary" className="w-1/2" onClick={() => onNext(
                                        { bookingTime: bookingTimes.find((time) => time.id.toString() === (selectedTime ?? "")) }
                                    )}>
                                        Tiếp theo
                                    </Button>
                                </div>
            ...
            <CardBody className="gap-4">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="text-right flex flex-col">
                        ...
                        <div>{dayjs(selectedTime?.start_date).format("DD/MM/YYYY")} - {dayjs(selectedTime?.end_date).format("DD/MM/YYYY")}</div>
                    </div>
                </div>
